...
- Added candle_type parameter for Binance API (v2.2.4)
- Improved handling of missing data (v2.2.5)
"""
import os
import json
import time
    ...
    def _download_all_pair_data(self):
        ...
        for pair in pairs:
            try:
                ...
                # First check if data exists to avoid unnecessary downloads
                try:
                    existing_data = load_pair_history(
                    ...
                    if len(existing_data) >= self.days_to_analyze:
                        logger.debug(f"Data already exists for {pair}, skipping download")
                        continue
                except:
                    pass
